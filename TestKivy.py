...
class New_Macauley:
    def __init__(self, distance, multiplier, magntiude, power, equation, unknown):
        self.distance = distance
...
class Macauley:
    def __init__(self, Load):
        ...
        elif Load.__class__.__name__ == "New_Macauley":
            self.multiplier = Load.multiplier
            self.power = Load.power
            self.equation = Load.equation
            self.magnitude = Load.magnitude
    ...
    def integrate(self):
        power = self.power+1
        multiplier = self.multiplier/(max(power, 1))
        equation = multiplier*self.magnitude*(x-self.distance)**power
        return New_Macauley(self.distance, multiplier, self.magnitude, power, equation, self.unknown)
    ...
    def differentiate(self):
        if self.power == 0:
            power = self.power - 1
            equation = 0
            return New_Macauley(self.distance,self.multiplier,self.magnitude,power,equation,self.unknown)
        else:
...
def Macauley_integrate(list, number):
    integrated_list = []
    for all in list:
        integrated_list.append(Macauley(all.integrate()))
    if number == 0:
...
def Macauley_differentiate(list):
    differentiated_list = []
    for all in list:
        differentiated_list.append(Macauley(all.differentiate()))
